...
function generateYears(dateUtils, year, noOfYear, minDate, maxDate) {
    var list = [];
    for (var i = 0; i < 2 * noOfYear + 1; i++) {
        ...
        if (minDate) {
            isInRange = dateUtils.getYear(minDate) <= newYear;
        }

        if (maxDate && isInRange) {
            isInRange = dateUtils.getYear(maxDate) >= newYear;
        }
...
class YearDropdownOptions extends React.Component {
    ...
    renderOptions = () => {
        ...
        var options = this.state.yearsList.map((year) => (
            ...
            >
                {selectedYear === year ? (
                    <span className="react-datepicker__year-option--selected">
                        ✓
                    </span>
                ) : (
                    ''
        ...
        const minYear = this.props.minDate
            ? this.props.dateUtils.getYear(this.props.minDate)
            : null;
        const maxYear = this.props.maxDate
            ? this.props.dateUtils.getYear(this.props.maxDate)
            : null;

        if (
            !maxYear ||
            !this.state.yearsList.find((year) => year === maxYear)
        ) {
            options.unshift(
                <div
        ...
        if (
            !minYear ||
            !this.state.yearsList.find((year) => year === minYear)
        ) {
            options.push(
                <div
                    className="react-datepicker__year-option"
...
export default withDateUtils(onClickOutside(YearDropdownOptions));
