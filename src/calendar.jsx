...
class Calendar extends React.Component {
    ...
    getDateInView = () => {
        ...
        const minDate = this.props.dateUtils.getEffectiveMinDate(this.props);
        const maxDate = this.props.dateUtils.getEffectiveMaxDate(this.props);
        const current = this.props.dateUtils.date();
        const initialDate = openToDate || selected || preSelection;
        if (initialDate) {
            return initialDate;
        } else {
            if (minDate && this.props.dateUtils.isBefore(current, minDate)) {
                return minDate;
            } else if (
                maxDate &&
                this.props.dateUtils.isAfter(current, maxDate)
            ) {
                return maxDate;
            }
        }
    ...
    increaseMonth = () => {
        this.setState(
            ({ date }) => ({
                date: this.props.dateUtils.addMonths(date, 1),
            }),
    ...
    changeYear = (year) => {
        this.setState(
            ({ date }) => ({
                date: this.props.dateUtils.setYear(date, year),
            }),
    ...
    changeMonth = (month) => {
        this.setState(
            ({ date }) => ({
                date: this.props.dateUtils.setMonth(date, month),
            }),
            () => this.handleMonthChange(this.state.date)
        );
    };
    ...
    changeMonthYear = (monthYear) => {
        this.setState(
            ({ date }) => ({
                date: this.props.dateUtils.setYear(
                    this.props.dateUtils.setMonth(
                        date,
                        this.props.dateUtils.getMonth(monthYear)
                    ),
                    this.props.dateUtils.getYear(monthYear)
                ),
            }),
    ...
    header = (date = this.state.date) => {
        const startOfWeek = this.props.dateUtils.getStartOfWeek(
            date,
            this.props.locale
        );
        const dayNames = [];
        ...
        return dayNames.concat(
            [0, 1, 2, 3, 4, 5, 6].map((offset) => {
                const day = this.props.dateUtils.addDays(startOfWeek, offset);
    ...
    formatWeekday = (day, locale) => {
        if (this.props.formatWeekDay) {
            return this.props.dateUtils.getFormattedWeekdayInLocale(
                day,
        ...
        return this.props.useWeekdaysShort
            ? this.props.dateUtils.getWeekdayShortInLocale(day, locale)
            : this.props.dateUtils.getWeekdayMinInLocale(day, locale);
    };

    decreaseYear = () => {
        this.setState(
            ({ date }) => ({
                date: this.props.dateUtils.addMonths(
                    date,
    ...
    renderPreviousButton = () => {
        ...
        switch (true) {
            case this.props.showMonthYearPicker:
                allPrevDaysDisabled = this.props.dateUtils.yearDisabledBefore(
                    this.state.date,
                    this.props
                );
                break;
            case this.props.showYearPicker:
                allPrevDaysDisabled = this.props.dateUtils.yearsDisabledBefore(
                    this.state.date,
                    this.props
                );
                break;
            default:
                allPrevDaysDisabled = this.props.dateUtils.monthDisabledBefore(
                    this.state.date,
                    this.props
                );
                break;
        ...
        return (
            <button
                ...
                aria-label={
                    isForYear ? previousYearAriaLabel : previousMonthAriaLabel
                }
            >
                {isForYear
    ...
    renderNextButton = () => {
        ...
        switch (true) {
            case this.props.showMonthYearPicker:
                allNextDaysDisabled = this.props.dateUtils.yearDisabledAfter(
                    this.state.date,
                    this.props
                );
                break;
            case this.props.showYearPicker:
                allNextDaysDisabled = this.props.dateUtils.yearsDisabledAfter(
                    this.state.date,
                    this.props
                );
                break;
            default:
                allNextDaysDisabled = this.props.dateUtils.monthDisabledAfter(
                    this.state.date,
                    this.props
                );
                break;
    ...
    renderYearDropdown = (overrideHide = false) => {
        ...
        return (
            <YearDropdown
                ...
                year={this.props.dateUtils.getYear(this.state.date)}
    ...
    renderMonthDropdown = (overrideHide = false) => {
        ...
        return (
            <MonthDropdown
                ...
                month={this.props.dateUtils.getMonth(this.state.date)}
    ...
    renderTodayButton = () => {
        ...
        return (
            <div
                ...
                onClick={(e) =>
                    this.props.onSelect(
                        this.props.dateUtils.getStartOfToday(),
                        e
                    )
                }
            >
    ...
    renderCustomHeader = (headerArgs = {}) => {
        ...
        const prevMonthButtonDisabled = this.props.dateUtils.monthDisabledBefore(
            this.state.date,
            this.props
        );

        const nextMonthButtonDisabled = this.props.dateUtils.monthDisabledAfter(
            this.state.date,
            this.props
        );

        const prevYearButtonDisabled = this.props.dateUtils.yearDisabledBefore(
            this.state.date,
            this.props
        );

        const nextYearButtonDisabled = this.props.dateUtils.yearDisabledAfter(
            this.state.date,
            this.props
        );
        ...
        return (
            <div className="react-datepicker__header react-datepicker-year-header">
                {showYearPicker
                    ...
                    : this.props.dateUtils.getYear(date)}
            </div>
        );
    };
    ...
    renderMonths = () => {
        ...
        var monthsToSubtract = this.props.showPreviousMonths
            ...
            : 0;
        var fromMonthDate = this.props.dateUtils.addMonths(
            this.state.date,
            monthsToSubtract * -1
        );
        for (var i = 0; i < this.props.monthsShown; ++i) {
            var monthsToAdd = i - this.props.monthSelectedIn;
            var monthDate = this.props.dateUtils.addMonths(
                fromMonthDate,
                monthsToAdd
            );
    ...
    renderInputTimeSection = () => {
        ...
        const timeValid =
            this.props.dateUtils.isValid(time) && Boolean(this.props.selected);
        const timeString = timeValid
            ? `${this.props.dateUtils.addZero(
                  this.props.dateUtils.getHours(time)
              )}:${this.props.dateUtils.addZero(
...
export default withDateUtils(onClickOutside(Calendar));
