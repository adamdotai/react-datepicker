...
function hasPreSelectionChanged(dateUtils, date1, date2) {
    if (date1 && date2) {
        return (
            dateUtils.getMonth(date1) !== dateUtils.getMonth(date2) ||
            dateUtils.getYear(date1) !== dateUtils.getYear(date2)
        );
    }
...
class DatePicker extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (prevProps.highlightDates !== this.props.highlightDates) {
            this.setState({
                highlightDates: this.props.dateUtils.getHightLightDaysMap(
    ...
    calcInitialState = () => {
        ...
        const boundedPreSelection =
            minDate &&
            this.props.dateUtils.isBefore(defaultPreSelection, minDate)
                ? minDate
                : maxDate &&
                  this.props.dateUtils.isAfter(defaultPreSelection, maxDate)
                ? maxDate
                : defaultPreSelection;
        return {
            ...
            dateFormat:
                ...
                this.props.dateUtils.formats.keyboardDate,
            dateFormatCalendar:
                this.props.dateFormatCalendar ||
                this.props.dateUtils.formats.monthAndYear,
        };
    };
    ...
    inputOk = () => this.props.dateUtils.isDate(this.state.preSelection);
    ...
    handleChange = (...allArgs) => {
        ...
        const date = this.props.dateUtils.parseDate(
            event.target.value,
    ...
    setSelected = (date, event, keepInput, monthSelectedIn) => {
        ...
        if (
            changedDate !== null &&
            this.props.dateUtils.isDayDisabled(changedDate, this.props)
        ) {
            return;
        }
        ...
        if (
            !this.props.dateUtils.isEqual(this.props.selected, changedDate) ||
            ...
            if (changedDate !== null) {
                if (
                    ...
                    changedDate = this.props.dateUtils.setTime(changedDate, {
                        hour: this.props.dateUtils.getHours(
                            this.props.selected
                        ),
                        minute: this.props.dateUtils.getMinutes(
                            this.props.selected
                        ),
                        second: this.props.dateUtils.getSeconds(
                            this.props.selected
                        ),
                    });
                }
            ...
            if (selectsRange) {
                ...
                if (noRanges) {
                    ...
                    if (this.props.dateUtils.isBefore(changedDate, startDate)) {
                        onChange([changedDate, null], event);
                    } else {
    ...
    setPreSelection = (date) => {
        ...
        if (date) {
            if (hasMinDate && hasMaxDate) {
                isValidDateSelection = this.props.dateUtils.isDayInRange(
                    date,
                    this.props.minDate,
                    this.props.maxDate
                );
            } else if (hasMinDate) {
                isValidDateSelection = this.props.dateUtils.isAfter(
                    date,
                    this.props.minDate
                );
            } else if (hasMaxDate) {
                isValidDateSelection = this.props.dateUtils.isBefore(
                    date,
                    this.props.maxDate
                );
            }
    ...
    handleTimeChange = (time) => {
        ...
        let changedDate = this.props.dateUtils.setTime(selected, {
            hour: this.props.dateUtils.getHours(time),
            minute: this.props.dateUtils.getMinutes(time),
        });
    ...
    onInputKeyDown = (event) => {
        ...
        if (this.state.open) {
            ...
            const copy = this.props.dateUtils.date(this.state.preSelection);
            if (eventKey === 'Enter') {
                event.preventDefault();
                if (
                    this.inputOk() &&
                    this.state.lastPreSelectChange ===
                        PRESELECT_CHANGE_VIA_NAVIGATE
                ) {
                    this.handleSelect(copy, event);
                    !this.props.shouldCloseOnSelect &&
                        this.setPreSelection(copy);
                } else {
                    this.setOpen(false);
                }
    ...
    onDayKeyDown = (event) => {
        ...
        if (eventKey === 'Enter') {
            ...
            switch (eventKey) {
                case 'ArrowLeft':
                    newSelection = this.props.dateUtils.addDays(copy, -1);
                    break;
                case 'ArrowRight':
                    newSelection = this.props.dateUtils.addDays(copy, 1);
                    break;
                case 'ArrowUp':
                    newSelection = this.props.dateUtils.addWeeks(copy, -1);
                    break;
                case 'ArrowDown':
                    newSelection = this.props.dateUtils.addWeeks(copy, 1);
                    break;
                case 'PageUp':
                    newSelection = this.props.dateUtils.addMonths(copy, -1);
                    break;
                case 'PageDown':
                    newSelection = this.props.dateUtils.addMonths(copy, 1);
                    break;
                case 'Home':
                    newSelection = this.props.dateUtils.addMonths(copy, -12);
                    break;
                case 'End':
                    newSelection = this.props.dateUtils.addMonths(copy, 12);
                    break;
    ...
    renderCalendar = () => {
        ...
        return (
            <Calendar
                ...
                dateFormat={this.state.dateFormatCalendar}
    ...
    renderDateInput = () => {
        ...
        const inputValue =
            typeof this.props.value === 'string'
                ...
                : this.props.dateUtils.safeDateFormat(this.props.selected, {
...
export default withDateUtils(DatePicker);
