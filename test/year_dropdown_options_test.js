...
describe('YearDropdownOptions with scrollable dropwdown', () => {
    ...
    it('should show upcoming and previous links and generate 10 years if prop scrollableYearDropdown is true', () => {
        ...
        expect(
            yearDropdown.find('.react-datepicker__navigation--years-upcoming')
                .length
        ).to.equal(1);
        expect(
            yearDropdown.find('.react-datepicker__navigation--years-previous')
                .length
        ).to.equal(1);
    });
    ...
    it('should generate years between minDate and maxDate if prop scrollableYearDropdown is true', () => {
        ...
        expect(yearDropdown.state().yearsList).to.contain(
            utils.getYear(minDate)
        );
        expect(yearDropdown.state().yearsList).to.contain(
    ...
    it('should show arrows to add years, if actual years list contains years between minDate and maxDate', () => {
        ...
        expect(textContents.find((year) => year === utils.getYear(minDate))).to
            .be.undefined;
        expect(textContents.find((year) => year === utils.getYear(maxDate))).to
            .be.undefined;
