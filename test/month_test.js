...
describe('Month', () => {
        ...
        range(0, dayCount).forEach((offset) => {
            ...
            assert(
                ...
                `Day ${(offset % 7) + 1} ` +
                    ...
                    `should be "${utils.formatDate(
                        expectedDay,
                        'yyyy-MM-dd'
                    )}" ` +
                    `but it is "${utils.formatDate(
                        day.props.day,
                        'yyyy-MM-dd'
                    )}"`
            );
    ...
    it('should render all days of the month and peek into the next month', () => {
        ...
        assertDateRangeInclusive(
            ...
            utils.getStartOfWeek(monthStart),
            utils.getEndOfWeek(
                utils.addWeeks(utils.addMonths(monthStart, 1), 1)
            )
        );
    });
    ...
    it('should have the quarter picker CSS class', () => {
        ...
        expect(month.hasClass('react-datepicker__quarterPicker')).to.equal(
            true
        );
    });
    ...
    it('should return disabled class if current date is out of bound of minDate and maxdate', () => {
        ...
        const quarter = monthComponent
            ...
            .at(0);
        expect(
            quarter.hasClass('react-datepicker__quarter--disabled')
        ).to.equal(true);
    });
    ...
    it('should return selected class if quarter is selected', () => {
        ...
        const quarter = monthComponent
            ...
            .at(0);
        expect(
            quarter.hasClass('react-datepicker__quarter--selected')
        ).to.equal(true);
    });
    ...
    it('should return quarter-in-range class if quarter is between the start date and end date', () => {
        ...
        const quarter = monthComponent
            ...
            .at(2);
        expect(
            quarter.hasClass('react-datepicker__quarter--in-range')
        ).to.equal(true);
    });
